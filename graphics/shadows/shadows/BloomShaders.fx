...
[numthreads(64, 1, 1)]
void bloomvertcs_main(uint3 groupId : SV_GroupID, uint3 threadId : SV_GroupThreadID)
{
    float4 curSum = float4(0, 0, 0, 0);
    for (int i = -HalfBlurSize; i <= HalfBlurSize; i++)
    {
        int2 pixCoord = clamp(int2(groupId.y * 64 + threadId.x, groupId.x * 64 + i), int2(0, 0), ImageSize);
        curSum += srcTexture0[pixCoord];
    }

    for (int i = 0; i < 64; i++)
    {
        int2 pixCoord = int2(groupId.y * 64 + threadId.x, groupId.x * 64 + i);
        dstTexture[pixCoord] = curSum / BlurSize;
        int2 prevPixCoord = max(int2(groupId.y * 64 + threadId.x, groupId.x * 64 + i - HalfBlurSize), int2(0, 0));
        int2 nextPixCoord = min(int2(groupId.y * 64 + threadId.x, groupId.x * 64 + i + HalfBlurSize + 1), ImageSize);
        curSum = curSum - srcTexture0[prevPixCoord] + srcTexture0[nextPixCoord];
    }
}
